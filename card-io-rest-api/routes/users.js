...
const { Card, addToLikedBy, removeFromLikedBy } = require("../models/card");
...
router.patch("/toggle-favorite/:cardId", auth, async (req, res) => {
  try {
    // add cardId to favorites
    let result = await User.updateOne(
      { _id: req.user._id },
      { $addToSet: { favorites: req.params.cardId } }
    );

    // if already existed before - remove from favorites
    if (result.nModified === 0) {
      await User.updateOne(
        { _id: req.user._id },
        { $pull: { favorites: req.params.cardId } }
      );
      await removeFromLikedBy(req.params.cardId, req.user._id);

      // if added to user's favorite - add to card's "liked_by"
    } else await addToLikedBy(req.params.cardId, req.user._id);

    res.send(`saved card ${req.params.cardId} to favorites`);
  } catch (ex) {
    console.log(ex);
